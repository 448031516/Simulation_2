...
public class DiscretePointUtil {
     ...
     */
    public static Point[] calMinMaxDots(final LinkedList<Point> points) {
        if (null == points || points.isEmpty()) {
            return null;
        }

        float min_x = points.get(0).getX(), max_x = points.get(0).getX();
        float min_y = points.get(0).getY(), max_y = points.get(0).getY();
    ...
    public static Point calCerter(Point ws, Point en) {
        ...
        return new Point((float) (ws.getX() + (en.getX() - ws.getX()) / 2), (float)(ws.getY()
                + (en.getY() - ws.getY()) / 2));
    }
