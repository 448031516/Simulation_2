...
public class KMeans {
    ...
    //簇的中心点
    List<Point2> centerPoints = new ArrayList<>();
    //聚类结果的集合簇，key为聚类中心点在centerPoints中的下标，value为该类簇下的数据点
    HashMap<Integer, List<Point2>> clusters = new HashMap<>();
    ...
    public void loadData(String path) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                points.add(new Point2(strs[0], strs[1]));
            }
            br.close();
    ...
    public void doKMeans(){
        ...
        while (err > 0.01 * cluterNum){
            ...
            for (int key : clusters.keySet()){
                List<Point2> list = clusters.get(key);
                list.clear();
    ...
    public void dispatchPointToCluster(Point2 point, List<Point2> centerPoints){
        int index = 0;
        ...
        List<Point2> list = clusters.get(index);
        list.add(point);
        clusters.put(index, list);
    }

    //计算每个类簇的中心点，并返回中心点偏移误差
    public double getClusterCenterPoint(List<Point2> centerPoints, HashMap<Integer, List<Point2>> clusters){
        double error = 0;
        for (int i=0; i<centerPoints.size(); i++){
            Point2 tmpCenterPoint = centerPoints.get(i);
            double centerX = 0, centerY = 0;
            List<Point2> lists = clusters.get(i);
            ...
            centerPoints.set(i, new Point2(centerX, centerY));
        }
        return error;
    }

    //计算点之间的距离，这里计算欧氏距离（不开方）
    public double calDistance(Point2 point1, Point2 point2){
        return Math.pow((point1.getX() - point2.getX()), 2) + Math.pow((point1.getY() - point2.getY()), 2);
    }

    //打印簇中心点坐标，及簇中其他点坐标
    public void show(List<Point2> centerPoints, HashMap<Integer, List<Point2>> clusters){
        for (int i=0; i<centerPoints.size(); i++){
